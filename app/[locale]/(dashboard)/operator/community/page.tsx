  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      <div className="mb-8">
        ...
        <p className="mt-1 text-sm text-gray-500">{t("community_subtitle")}</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
        <div className="relative w-full sm:w-96">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#7B5EA7]/20 focus:border-[#7B5EA7]"
          />
        </div>
        <div className="flex gap-2 w-full sm:w-auto">
          <button 
            onClick={() => setFilter("all")}
            className={`px-4 py-2 text-sm font-medium rounded-xl flex-1 sm:flex-none ${filter === "all" ? "bg-[#7B5EA7] text-white" : "bg-gray-50 text-gray-600 hover:bg-gray-100"}`}
          >
            {t("filter_all")}
          </button>
          <button 
            onClick={() => setFilter("hidden")}
            className={`px-4 py-2 text-sm font-medium rounded-xl flex-1 sm:flex-none ${filter === "hidden" ? "bg-[#7B5EA7] text-white" : "bg-gray-50 text-gray-600 hover:bg-gray-100"}`}
          >
            {t("filter_hidden")}
          </button>
        </div>
                ...
                filteredPosts.map((post) => (
                  <tr key={post.id} className="hover:bg-gray-50/50 transition-colors">
                    <td className="px-6 py-4">
                      <p className="font-semibold text-gray-900 max-w-xs truncate" title={post.title}>{post.title}</p>
                      <p className="text-xs text-gray-500 mt-1 max-w-xs truncate" title={post.content}>{post.content}</p>
                    </td>
                    <td className="px-6 py-4">
